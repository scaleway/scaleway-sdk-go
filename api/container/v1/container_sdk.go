...
// GetPrivateEndpointCertificateAuthorityRequest: get private endpoint certificate authority request.
type GetPrivateEndpointCertificateAuthorityRequest struct {
	// Region: region to target. If none is passed will use default region from the config.
	Region scw.Region `json:"-"`
}
...
// GetPrivateEndpointCertificateAuthority: When enabling private endpoints for your containers, you need to trust this CA to establish HTTPS connections to them.
func (s *API) GetPrivateEndpointCertificateAuthority(req *GetPrivateEndpointCertificateAuthorityRequest, opts ...scw.RequestOption) (*scw.File, error) {
	var err error
	...
	scwReq := &scw.ScalewayRequest{
		Method: "GET",
		Path:   "/containers/v1/regions/" + fmt.Sprint(req.Region) + "/private-endpoint-ca",
	}
