...
// Package payment_private provides methods and message types of the payment_private v2alpha2 API.
...
const (
	PaymentMeansUnknown = PaymentMeans("unknown")
	PaymentMeansCard    = PaymentMeans("card")
	PaymentMeansSepa    = PaymentMeans("sepa")
)

func (enum PaymentMeans) String() string {
	if enum == "" {
		// return default value if empty
		return "unknown"
	}
...
func (enum PaymentMeans) MarshalJSON() ([]byte, error) {
	return []byte(fmt.Sprintf(`"%s"`, enum)), nil
}

func (enum *PaymentMeans) UnmarshalJSON(data []byte) error {
	tmp := ""
	...
	*enum = PaymentMeans(PaymentMeans(tmp).String())
	return nil
}
...
type MagicCode struct {
	...
	// PaymentMeans: default value: unknown
...
type ConsoleAPIListMagicCodesRequest struct {
	...
	// PaymentMeans: default value: unknown
	PaymentMeans PaymentMeans `json:"-"`
}
...
func (s *ConsoleAPI) SendMagicCode(req *ConsoleAPISendMagicCodeRequest, opts ...scw.RequestOption) (*MagicCode, error) {
	...
	scwReq := &scw.ScalewayRequest{
		Method: "POST",
		Path:   "/payment-private/v2alpha2/magic-codes",
	}
...
func (s *ConsoleAPI) ValidateMagicCode(req *ConsoleAPIValidateMagicCodeRequest, opts ...scw.RequestOption) (*MagicCode, error) {
	...
	scwReq := &scw.ScalewayRequest{
		Method: "POST",
		Path:   "/payment-private/v2alpha2/magic-codes/" + fmt.Sprint(req.MagicCodeID) + "/validate",
	}
...
func (s *ConsoleAPI) ListMagicCodes(req *ConsoleAPIListMagicCodesRequest, opts ...scw.RequestOption) (*ListMagicCodesResponse, error) {
	...
	parameter.AddToQuery(query, "device_id", req.DeviceID)
	parameter.AddToQuery(query, "status", req.Status)
	parameter.AddToQuery(query, "organization_id", req.OrganizationID)
	parameter.AddToQuery(query, "payment_means", req.PaymentMeans)
