...
type DomainStatusRecordStatus string

const (
	// Default unknown domain record status
	DomainStatusRecordStatusUnknownRecordStatus = DomainStatusRecordStatus("unknown_record_status")
	// The record is valid
	DomainStatusRecordStatusValid = DomainStatusRecordStatus("valid")
	// The record is invalid
	DomainStatusRecordStatusInvalid = DomainStatusRecordStatus("invalid")
	// The record was not found
	DomainStatusRecordStatusNotFound = DomainStatusRecordStatus("not_found")
)

func (enum DomainStatusRecordStatus) String() string {
	if enum == "" {
		// return default value if empty
...
func (enum DomainStatusRecordStatus) MarshalJSON() ([]byte, error) {
	return []byte(fmt.Sprintf(`"%s"`, enum)), nil
}

func (enum *DomainStatusRecordStatus) UnmarshalJSON(data []byte) error {
	tmp := ""
	...
	*enum = DomainStatusRecordStatus(DomainStatusRecordStatus(tmp).String())
	return nil
}
...
type DomainStatus struct {
	...
	SpfRecord *DomainStatusSpfRecord `json:"spf_record"`
	// DkimRecord: the DKIM record verification data.
	DkimRecord *DomainStatusDkimRecord `json:"dkim_record"`
}

// DomainStatusDkimRecord: domain status. dkim record.
type DomainStatusDkimRecord struct {
	// Status: status of the DKIM record's configurartion.
	// Default value: unknown_record_status
	Status DomainStatusRecordStatus `json:"status"`
...
// DomainStatusSpfRecord: domain status. spf record.
type DomainStatusSpfRecord struct {
...
// CheckDomainStatus: display SPF and DKIM records status and potential errors.
// Display SPF and DKIM records status and potential errors, including the found records to ease the debugging.
func (s *API) CheckDomainStatus(req *CheckDomainStatusRequest, opts ...scw.RequestOption) (*DomainStatus, error) {
	var err error
